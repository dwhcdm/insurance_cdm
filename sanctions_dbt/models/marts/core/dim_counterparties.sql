...
/*
    Dimension: Counterparties with screening summary and composite risk tier.
    Enriched with screening metrics for 360-degree counterparty risk view.
*/
...
final AS (
    ...
    SELECT
        {{ hash_surrogate_key(['c.counterparty_id']) }} AS counterparty_sk,
        c.counterparty_id,
        c.legal_name,
        c.entity_type,
        ...
        CASE
            ...
            WHEN COALESCE(ss.sar_filed_count, 0) > 0 THEN 'CRITICAL'
            WHEN c.risk_rating = 'HIGH' OR COALESCE(ss.true_positive_count, 0) > 0 THEN 'HIGH'
    ...
    LEFT JOIN screening_summary ss ON c.counterparty_id = ss.counterparty_id
